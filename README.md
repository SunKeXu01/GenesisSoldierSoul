...
- `playable_maps` 仍记录 19 个原始客户端缺失 GUID；当前可玩场景使用已审计的恢复与
  生成闭包，但这些缺口会继续阻止正式资源门禁放行。
- CF 私有目录 REZ、高熵 LTC、Flash ATF，以及使用 Oodle/IoStore 的 Unreal 容器尚未
  完成内容级转换；工具只做静态读取，不执行来源不明程序，也不猜测密钥。
