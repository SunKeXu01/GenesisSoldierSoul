...
- 专用格式转换已建立统一证据账本：2 份 UnityPackage 的 19 个资产保持 GUID、原路径与 `.meta`；417 个 SWF/误标缓存共提取 1,874 个 PNG、1,601 个矢量标签、74 个字体标签、1,380 个 Sprite/时间轴和 538 个 ATF；15 个原始 OBJ/FBX/3DS 转为 GLB、8 个 BMP/TGA 转为 PNG、188 个 MP3 转为 PCM WAV。标准 `engine.rez` 的 DTX 已转 PNG，15 个 LTB 确认为 RenderStyle 并保留结构证据。综合账本逐项复核 50,676 个输出、1,497,484,666 字节，错误 0，见 `recovery/SPECIAL_FORMAT_CONVERSION_LEDGER_2026-08-04.md`。
...
- CF 私有目录的 `RF*.REZ`、高熵 LTC，以及这些私有容器内尚不可访问的 DTX/LTB；标准 `engine.rez` 的 DTX 已转 PNG，其中 15 个 LTB 实为 RenderStyle 而非几何模型。
