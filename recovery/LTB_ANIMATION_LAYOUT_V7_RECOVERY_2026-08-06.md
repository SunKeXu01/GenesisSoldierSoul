...
- 统一 provenance：12/12 清单、148,829 个输出、21,546,511,271 字节、错误 0。

## 边界

glTF 已含骨骼 TRS 动画、vertex morph 动画和 string-key extras；socket、animation weight-set 混合语义与外部 child-model 运行时绑定仍只做严格审计，未伪装成标准 glTF 行为。REZ 剩余范围为 5 个完全未分帧包和两份 RF199 未知后缀。
